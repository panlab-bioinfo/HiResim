...
#首先要得到每个strain的GTDB参考基因组，然后把N替换，拼接
#然后得到基因组的长度，根据ani计算变异的碱基数目
...
path_strain_genome = os.path.join(path_home,"data/strain_download") #下载的基因组的路径
for n_index,ps in enumerate(path_sp_list): #ps sp_strain_abu_10.csv  or sp_strain_abu.csv
    if len(path_sp_list)>1:
        ps = "sp_strain_abu_"+str(n_index+1)+".csv"
    ...
    #strain的abu,名字和是一样的，old的abu.pkl是存的所有样本的，new中一个样本一个pkl,直接用csv吧
    ...
    df_strain["abu_new"] = df_strain["abu"].apply(lambda x:float(x/sum_abu))
    # print(df_strain)
    #这个包含了模拟strian的丰度，明天需要根据sp下的strain个数和ani生成新的strain
    path_strain_df = os.path.join(sim_out,"sim_strain"+str(n_index+1)+".txt")
    ...
    path_tmp_strain = os.path.join(sim_out,"strain_download_"+str(n_index+1)+".txt")
    ...
    for index_s,ls in enumerate(list_sp): 
        ...
        out_file_sp = os.path.join(path_strain_sim,ls) 
